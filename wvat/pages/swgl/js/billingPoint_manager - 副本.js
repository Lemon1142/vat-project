...
var colMap ={
	...
	colModel : [
	    ...
	    {name:'sellerTaxNo',index:'sellerTaxNo', width:"16%", align:'center', sortable:false},
	    {name:'sellerName',index:'sellerTaxNo', width:"14%",align:'center', sortable:false},
	    {name: '操作', index:"operation", width:'10%', align:'left', sortable:false, formatter:function (value,gird,rows,state)
	    	{
				var mac = rows.mac;
				return "<span data-method='btn_look' class='operation_icon btn-ev fa fa-file-text " + (cbtnArr.indexOf('201050') == -1 ? 'lock_hide' : 'lock_show' ) + "' title='查看'></span>" +
					   "<span data-method='btn_edit' class='operation_icon btn-ev fa fa-pencil-square-o " + (cbtnArr.indexOf('201030') == -1 ? 'lock_hide' : 'lock_show' )+"' title='编辑'></span>" +
					   "<span data-method='btn_delete1' class=' operation_icon btn-ev fa fa-trash " + (cbtnArr.indexOf('201040') == -1 ? 'lock_hide' : 'lock_show' ) + "' title='删除'></span>" +
					   "<span data-method='btn_upload' class=' operation_icon btn-ev fa fa-upload " + ( ( (cbtnArr.indexOf('201070') != -1) && (!isEmpty(mac)) ) ? 'lock_show' : 'lock_hide' ) + "' title='推送'></span>"
	    ...
	    {name:'billingPointName', index:'billingPointName', width:"15%", align:'center', sortable:false},
		...
		if( $(this).hasClass("layui-this") ){
			...
			if(num == '1'){
				if(code == ""){
					...
					layer.msg('输入的发票代码应为自然数！', {zIndex: layer.zIndex});
					return false;
				};
				if(number == "" ){
					...
					layer.msg('输入的发票号码应为自然数！', {zIndex: layer.zIndex});
					return false;
				};
				...
				if(start == "" ){
					...
					layer.msg('输入的发票号码起应为自然数！', {zIndex: layer.zIndex});
					return false;
				};
				if(end == "" ){
					...
					layer.msg('输入的发票号码止应为自然数！', {zIndex: layer.zIndex});
					return false;
				};
