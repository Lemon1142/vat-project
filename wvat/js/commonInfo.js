...
function getTopWinow(){
 ...
 //存储cookie wvat.gooagoo.com  wvat.test.goago.cn wvat.dev.goago.cn
function setCookie(c_name, value, expiredays) {
	 var exdate = new Date()
	 exdate.setDate(exdate.getDate() + expiredays)
	 document.cookie = c_name + "=" + escape(value) +
		 //((expiredays == null) ? "" : ";expires=" + exdate.toGMTString()) + ';domain='+'goago.cn';
		 ((expiredays == null) ? "" : ";expires=" + exdate.toGMTString()) + ';domain= '+ window.gooagoodomain ;
}
...
function readGridCookie(gridId){
	...
	if(arr=document.cookie.match(reg)){
		...
		for(var i=0; i< showColRowIds.length; i++){
			//此处不要用.width(colWidthArray[i-3])方法，以免设置的宽度和实际有偏差
			$("#" + gridId + "_" + showColNames[i]).css("width", colWidthArray[i] + "px");
			$("#" + gridId + " td:eq(" + showColRowIds[i] + ")").css("width", colWidthArray[i] + "px");
		}
...
function writeGridCookie(gridId, colWidthArray, hideColRowIds){
    ...
    //expiresDate.setTime(expiresDate.getTime() );//10*365*24*60*60*1000《十年》如果那两个字没有颤抖……
	...
	$("#gridCookieDiv" + "_" + gridId).dialog({
		...
		buttons : {
			...
			"确定" : function(){
				...
				var totalWidth = 0;
				var i = 0;
				for(var rowId = 0; rowId< allRowIds.length; rowId++){
				...
				//	//var rowData = $("#gridCookieDiv" + "_" + gridId + "_table").getRowData(allRowIds[rowId]);
				//	console.log('boolean',allRowIds.hasOwnProperty(rowId));
				//	if( allRowIds.hasOwnProperty(rowId)){
				//		var rowData = $("#gridCookieDiv" + "_" + gridId + "_table").getRowData(allRowIds[rowId]);
				//		if(allRowIds[rowId] == selectRowIds[i]){
				//			totalWidth += parseInt(rowData.gridCookieColWidth);
				//			colWidthArray.push(rowData.gridCookieColWidth);
